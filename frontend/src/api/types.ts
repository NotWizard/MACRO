...
export interface IndexDividendMetric {
  value: number | null
  date: string | null
  /** 当前值来源：official（中证官方锚）/ recon（TR/PR 重建）/ danjuan（蛋卷）/ derived */
  value_source: string | null
  /** higher_is_expensive（PE/PB）/ higher_is_cheap（股息率/利差）/ neutral */
  direction: 'higher_is_expensive' | 'higher_is_cheap' | 'neutral'
  presets: Record<string, PercentileResult>
  custom: PercentileResult | null
}

export interface IndexDividendSummary {
  index_code: string
  index_name: string
  latest_date: string | null
  caliber: string | null
  k_dy: number | null
  metrics: Record<string, IndexDividendMetric>
}

export interface IndexDividendGate {
  gate: string
...
export interface IndexDividendHealth {
  gates: IndexDividendGate[]
  sources: { table: string; label: string; latest_date: string | null; rows: number }[]
}
